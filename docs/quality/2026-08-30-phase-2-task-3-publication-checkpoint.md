...
The independent security review then supplied targeted regressions for each of the five findings below, exercising either the corrected behavior or its fail-closed rejection boundary. The ambient-credential regression specifically combines a poisoned credential path with the unsafe REST preference and verifies rejection before Firebase Admin construction, without credential-path disclosure or state mutation. Those regressions remain in the focused suites; strengthening them did not inflate the final top-level publication count because related assertions were kept within the owning lifecycle cases.
...
The independent security review identified five concrete findings. All five were fixed and have targeted regression coverage:

1. **REST fallback could consult ambient ADC in emulator mode.** The command now rejects a forced REST preference before Firebase Admin construction, constructs the accepted emulator Firestore client with `preferRest: false`, and uses only the exact emulator project tuple. The regression combines the rejected REST preference with a poisoned ambient credential path and verifies the fixed error, no Firebase Admin import, no credential-path disclosure, and no state mutation; it does not claim to exercise a successful apply with poisoned ADC.
...
The final security verdict is **no open finding within the emulator-only Task 3 scope**. The controls prove that the accepted path scans the bytes it parses and cannot escape the local emulator, the unsafe REST path is rejected before Firebase construction with no observed Admin import, credential-path disclosure, or state mutation, corrupt history fails closed, and foreign diagnostic data is not exposed.
