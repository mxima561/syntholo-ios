...
The current app is not at this point. Phase 0 is verified, Phase 1 is substantially complete, and Phases 2–9 remain. Tasks 9–10 now install and open an exact immutable read-only preview route in the same onboarding session, exercise the deterministic curriculum UI matrix without constructing live Firebase/Firestore dependencies, and audit the fresh synthetic catalog/preview. This is still only a preview seam: there is no real lesson-start mutation, interaction, feedback, completion, progress, approved original launch curriculum, staging proof, or privacy-safe curriculum analytics. Task 11 is next.
...
3. Preserve the completed emulator-only publisher/rollback, exact-read Rules, Swift domain/digest-parity, app-owned cache, exact-get repository, Task 8 observable-store/read-only-UI, Task 9 root-composition/same-session-preview, and Task 10 deterministic UI/no-Firestore/accessibility checkpoints; build Task 11's privacy-safe curriculum analytics next.
